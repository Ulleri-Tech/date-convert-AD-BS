import { numberOfDaysEachYearBS, numberOfDaysEachMonthBS, MAX_DAY_DIFF, MIN_AD_YEAR, MIN_AD_MONTH, MIN_AD_DAY, MILLS_IN_DAY, MIN_BS_YEAR, weekDayBS, MONTH_NAME_BS } from "./constant";
...
export function BSTOAD(year: number, month: number, day: number):string {
    ...
    dayCount += day_num - 1;
    const final_time = Math.ceil((new Date(MIN_AD_YEAR, MIN_AD_MONTH, MIN_AD_DAY).getTime())) + (dayCount * MILLS_IN_DAY)
...
export function dateTodayBS():string {
    ...
    return `${weekday_BS}, ${MONTH_NAME_BS[+month_bs - 1]} ${day_bs}, ${year_bs} बि.सं`
}

export function dateTodayAD() {
